...
from Map_Extraction.GeoJSONHandler import GeoJSONOpener
import osmnx as ox
...
class Street:
    ...
    def getBearings(self):
        ...
        #Let's obtain the bearings
        for i in range(0,len(formatedPoints)):
            point = [formatedPoints[i][1], formatedPoints[i][0]]
            closestLine = self.getClosestLine(point)
            point1 = np.array(closestLine.point1)
            point2 = np.array(closestLine.point2)
            refPoint = point1 + deltaFraction * (point2 - point1)

            bearing = calculator.getBearing(point[1], point[0], refPoint[1], refPoint[0])
    ...
    def openStreetsWithDataFrame(self, dataFrame):
        ...
        for i in range(0,nSegments):
            ...
            if(streetName == None or (type(streetName) == float and math.isnan(streetName))):
                streetName = "unnamed_"+str(unknownCounter)
                unnamed = True
            else:
                unnamed = False
